...
void CPlot2D::Draw(CDC& dc, int Ind1, int Ind2)		// Рисование с самостоятельным пересчетом координат
{
	...
	if (Ind2 == 1)
	{
		...
		// ====== AXIS TICKS AND LABELS ADDED ======

		int tickLen = 5;
		int fontSize = 8;
		double stepX = (RS.right - RS.left) / 10;
		double stepY = (RS.top - RS.bottom) / 10;
		...
		{
			for (double x = RS.left; x <= RS.right; x += stepX)
			{
				double xs = x, ys = 0;
				int xw, yw;
				GetWindowCoords(xs, ys, xw, yw);

				dc.MoveTo(xw, yw - tickLen);
				dc.LineTo(xw, yw + tickLen);

				CString text;
				text.Format(L"%.1f", x);
				dc.TextOut(xw + 2, yw + 5, text);
			}
		}

		if (RS.left * RS.right < 0)
		{
			for (double y = RS.bottom; y <= RS.top; y += stepY)
			{
				double xs = 0, ys = y;
				int xw, yw;
				GetWindowCoords(xs, ys, xw, yw);

				dc.MoveTo(xw - tickLen, yw);
				dc.LineTo(xw + tickLen, yw);

				CString text;
				text.Format(L"%.1f", y);
				dc.TextOut(xw + 5, yw - 10, text);
			}
		}
		...
		// ---- ПОДПИСИ ОСЕЙ X и Y ----
		{
			...
			int xw, yw;
			GetWindowCoords(xs, ys, xw, yw);
			dc.TextOut(xw + 50, yw + 15, L"X");

			// Подпись Y
			xs = 0;
			ys = RS.top;
			GetWindowCoords(xs, ys, xw, yw);
			dc.TextOut(xw - 15, yw - 50, L"Y");
		}
